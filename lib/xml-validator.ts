...
> {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return { valid: true, parsed, aasData } // Fallback to valid if schema fetch fails
    }
    ...
    console.error("[v0] Schema validation error (external service issue):", err.message)
    // Fall back to structure validation result if external service fails
    const aasData = extractAASDataFromXML(parsed)
    console.log("[v0] ===== XML VALIDATION END (PASSED - FALLBACK TO STRUCTURE) =====")
    return { valid: true, parsed, aasData } // Fallback to valid if external service fails
  }
}